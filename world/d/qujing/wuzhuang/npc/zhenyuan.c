...
void send_back(object me, object who);

string KEZHAN="/d/city/kezhan";
...
void init()
{
	...
	if(this_object()->query_temp("invite")) {
	    if(this_player()->query("life/live_forever")) {
		call_out("send_back",0,this_object(), this_player());
	    }
	}
}
...
{ 
	...
	for(i=0; i<sizeof(inv); i++)
	{
		if( userp(inv[i]) && (!wizardp(inv[i])) 
		 && !inv[i]->query_temp("netdead") ) total++;
	}
...
void inviting (object me)
{
	...
	while  (i--)
	{
		...
		// mon 7/25/99
		if(list[i]->query("life/live_forever"))
		    continue;

    // add age limit 
     if (list[i]->query("age")<15) continue;
  ...
  if (query("guests/"+list[i]->query("id"))) {
   tell_object(list[i],"你耳边突然传来镇元大仙愤怒的声音：“怎么又是你？！上上下下，你当这里是ＳＨＵＴＴＬＥ　ＢＵＳ啊？\n");
    message_vision("半空中有人飞起一脚，把$N从云端中踹了下去。\n",list[i]);
  return ;}
  set("guests/"+list[i]->query("id"),1);
    		list[i]->move(here);
...
void send_back(object me, object who)
{
	...
	//    there = who->query_temp("wzg/from");
	who->set_temp("accept",0);
	message_vision("$N的大袖朝着$n轻轻一罩…\n",me,who);
	result=who->move(KEZHAN);

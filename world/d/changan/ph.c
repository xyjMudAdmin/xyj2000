...
//   {  message_vision("看门老头一把拉住$N的衣服，兴奋地说：“"+me->name()+"，这下可逮着你了！还不快付欠下的管理费！”\n",me);me->start_busy(1);return 1;}
    if (!me->query_temp("armor/cloth")) return notify_fail("衣冠不整，恕不接待。\n");
    if (me->query_temp("weapon")) return notify_fail("手持凶器，你是去做客还是去打劫啊？\n");
  }

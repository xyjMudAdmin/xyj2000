...
void announce_live_forever(string reason)
{
    ...
    if( !me->query("fake_age") ) me->set("fake_age",me->query("age"));
...
varargs int receive_wound(string type, int damage, object who)
{
	...
	if( val >= 0 ) set("eff_" + type, val);
	    ...
	    if(val1>100) { //if val1 still high, only faint, 
		           //not die immediately. -mon 4/20/99
		set( "eff_" + type, 0);
		set(type,-1);
		val1=-1;
		val=0;
...
void die()
{
	...
	// Clear temp makeup age. -- xlb && mon, 6/13/99
	delete_temp("age_modify");

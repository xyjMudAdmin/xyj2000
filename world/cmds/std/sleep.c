...
int main(object me, string arg)
{
	...
	if(me->query("sen")<1 ||
		me->query("eff_sen")<1) 
	  return notify_fail("你现在精神太差，一睡倒恐怕就再也醒不过来了。\n");

	if(me->query("kee")<1 ||
		me->query("eff_kee")<1) 
	  return notify_fail("你现在气血不足，一睡倒恐怕就再也醒不过来了。\n");

        if((!arg)||arg==(string)me->query("id")) {
          string sleep_msg;
	  int ugly=1;
	  
	  if(random(me->query_per())>20) { // per larger better here.
	      ugly=0;
	  } else if(random(me->query_str())<5) { // str smaller better here.
	      ugly=0;
	  } else if((me->query("gender")=="女性") && (random(5)>0)) {
	      ugly=0;
	  }

	  if(ugly) {
	      sleep_msg=({"不一刻便鼾声大作，进入了梦乡。\n",
		      "不一会儿便鼾声大作，沉睡过去。\n",
		      "如雷的鼾声立刻响了起来。\n",
		      "埋头睡去。\n",
		      })[random(2)];
	  } else {
	      sleep_msg=({"不一会儿就甜甜地进入了梦乡。\n",
			  "不一会儿就静静地进入了梦乡。\n",
			  "进入了甜蜜的梦乡。\n",
			  })[random(2)];
	  }
	  ...
	  if (where->query("if_bed")) {
		...
		message_vision(HIY "$N往被中一钻，"+sleep_msg+ NOR,me);
	  } else { 
		write("你就地一躺，开始睡觉。\n");
		write("不一会儿，你就进入了梦乡。\n");
		me->set("last_sleep",time());
		me->set_temp("block_msg/all",1);
		message_vision(HIY "$N就地一躺，"+sleep_msg+ NOR,me);
	  }
